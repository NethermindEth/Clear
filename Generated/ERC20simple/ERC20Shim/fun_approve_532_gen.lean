...
def fun_approve_532 : FunctionDefinition := <f
    function fun_approve_532(var_owner, var_spender, var_value, var_emitEvent) -> 
    
{
    ...
    let _104_slot := 1
    let expr_514_slot := _104_slot
    let _22 := var_owner
    let expr_7 := var_owner
    let _23 := mapping_index_access_mapping_address_mapping_address_uint256_of_address(_104_slot, var_owner)
    let _107_slot := _23
    let expr_517_slot := _23
    let _24 := var_spender
...
def fun_approve_532_concrete_of_code
: {
    C :
      _ → _ → _ → _ → 
      State → State → Prop
