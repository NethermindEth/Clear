  ...
  if expr_5 
{
    ...
    let expr_17 := 0
    fun_approve_532(var_owner, var_spender, expr_16, expr_17)
}
>
...
def if_715180968049861219_concrete_of_code : {
    C : State → State → Prop
    // ∀ {s₀ s₉ fuel}
    , exec fuel if_715180968049861219 s₀ = s₉
    → Spec C s₀ s₉
  } := by
  constructor
  intros s₀ s₉ fuel

  unfold Spec if_715180968049861219
  ...
  intros h
  try intros h'
  refine' Exists.intro s (And.intro (fun_approve_532_abs_of_code hs) ?_)
  swap; clear hs
  try revert h'
  revert h
