  ...
  if expr_2 
{
    ...
    var_newFrom := checked_sub_uint256(var_from, var_value)
    let _6 := var_to
    let expr_5 := var_to
    let _7 := var_value
    let expr_6 := var_value
    var_newTo := checked_add_uint256(var_to, var_value)
    var_success := 1
}
>
...
def if_668760047301878650_concrete_of_code : {
    C : State → State → Prop
    // ∀ {s₀ s₉ fuel}
    , exec fuel if_668760047301878650 s₀ = s₉
    → Spec C s₀ s₉
  } := by
  constructor
  intros s₀ s₉ fuel

  unfold Spec if_668760047301878650
