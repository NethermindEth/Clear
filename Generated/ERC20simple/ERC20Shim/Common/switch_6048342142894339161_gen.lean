...
def switch_6048342142894339161 := <s
  switch expr_17 case 0 {
    let _32 := var_value
    let expr_18 := var_value
    let _slot := 0
    let expr_372_slot := _slot
    let _33 := var_to
    let expr_19 := var_to
    let _34 := mapping_index_access_mapping_address_uint256_of_address(_slot, var_to)
    let _35 := sload(_34)
    let _36 := _35
...
def switch_6048342142894339161_concrete_of_code : {
    C : State → State → Prop
    // ∀ {s₀ s₉ fuel}
    , exec fuel switch_6048342142894339161 s₀ = s₉
    → Spec C s₀ s₉
  } := by
  constructor
  intros s₀ s₉ fuel

  unfold Spec switch_6048342142894339161
