...
def switch_8694440494872382586 := <s
  switch expr_1 case 0 {
    let _60_slot := 0
    let expr_334_slot := _60_slot
    let _4 := var_from
    let expr_2 := var_from
    let _5 := mapping_index_access_mapping_address_uint256_of_address(_60_slot, var_from)
    let _6 := sload(_5)
    let _7 := _6
    let expr_3 := _6
    ...
    let _73_slot := _60_slot
    let expr_349_slot := _60_slot
    let _22 := var_from
    let expr_13 := var_from
    let _23 := mapping_index_access_mapping_address_uint256_of_address(_60_slot, var_from)
...
def switch_8694440494872382586_concrete_of_code : {
    C : State → State → Prop
    // ∀ {s₀ s₉ fuel}
    , exec fuel switch_8694440494872382586 s₀ = s₉
    → Spec C s₀ s₉
  } := by
  constructor
  intros s₀ s₉ fuel

  unfold Spec switch_8694440494872382586
