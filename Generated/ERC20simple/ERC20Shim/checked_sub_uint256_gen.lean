...
def checked_sub_uint256 : FunctionDefinition := <f
...
def checked_sub_uint256_concrete_of_code
: {
    C :
      _ → _ → _ → 
      State → State → Prop
    // ∀ {s₀ s₉ : State} {diff x y fuel},
         execCall fuel checked_sub_uint256 [diff] (s₀, [x, y]) = s₉ →
         Spec (C diff x y) s₀ s₉
  } := by
  constructor
  intros s₀ s₉ diff x y fuel
  unfold checked_sub_uint256
