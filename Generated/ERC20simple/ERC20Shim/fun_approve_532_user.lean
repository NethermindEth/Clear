...
def A_fun_approve_532  (var_owner var_spender var_value var_emitEvent : Literal) (s₀ s₉ : State) : Prop := sorry

lemma fun_approve_532_abs_of_concrete {s₀ s₉ : State} { var_owner var_spender var_value var_emitEvent} :
  Spec (fun_approve_532_concrete_of_code.1  var_owner var_spender var_value var_emitEvent) s₀ s₉ →
  Spec (A_fun_approve_532  var_owner var_spender var_value var_emitEvent) s₀ s₉ := by
  unfold fun_approve_532_concrete_of_code A_fun_approve_532
  sorry
